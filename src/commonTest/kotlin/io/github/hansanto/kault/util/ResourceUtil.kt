...
fun String.asResourceFile(): Path {
    return workingDirectory().resolve("src/commonTest/resources/$this")
}
...
inline fun <reified T> readJson(name: String): T {
    return VaultClient.json.decodeFromString(readResourceFile(name))
}

fun readResourceFile(name: String): String {
    return name.asResourceFile().readLines()
}
