...
class StringExtTest :
    ShouldSpec({
        ...
        should(
            "add url child path with non-empty parent and non-empty child with leading slash and no trailing slash"
        ) {
            val parent = "http://localhost:8200/"
            val child = "/v1"
            val expected = "http://localhost:8200/v1"
        ...
        should(
            "add url child path with non-empty parent and non-empty child with no leading slash and trailing slash"
        ) {
            val parent = "http://localhost:8200"
            val child = "v1/"
            val expected = "http://localhost:8200/v1"
